...
function UpdateMenu() {
    ...
    const [mainMenus, setMainMenus] = useState(['']);
    const [subMenus, setSubMenus] = useState(['', '']);
    const maxSubMenus = 10;
    const maxMainMenus = 2;
        ...
        if (id) {
            axios.get(`/menu/${id}`, {
                ...
                .then(response => {
                    ...
                    setYear(new Date(menu.mealDate).getFullYear());
                    setMonth(new Date(menu.mealDate).getMonth() + 1);
                    setDate(new Date(menu.mealDate).getDate());
                    setMainMenus(menu.mainMenus || ['']);
